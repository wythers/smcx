...
export class Echo implements Contract {
    ...
    async sendTON(provider: ContractProvider, via: Sender, value: bigint, comment: string = "") { 
        await provider.internal(via, {
            value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(OpCode.Transfer, 32).storeStringTail(comment).endCell()
        });
    }
}
